...
async def report(
    ...
    for file in files:
        ...
        out_html = os.path.join(reports_subdir, f"{basename}.html")
        ...
        sql_text_for_excerpt = base_query.strip().replace("\n", " ")
        items_for_index.append(IndexItem(
            title=Path(file).name,
            file=str(Path(file)),
            report_rel=rel_path,
            risk=("HIGH" if getattr(profile, "est_pages", 0) >= 500_000 or getattr(profile, "est_memory_bytes", 0) >= 1_000_000_000
                else "MED" if getattr(profile, "est_pages", 0) >= 100_000 or getattr(profile, "est_memory_bytes", 0) >= 256_000_000
                else "LOW"),
